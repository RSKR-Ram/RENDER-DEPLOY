...
export function TrainingCard({ item, canUpdate, onStart, onComplete }) {
    ...
    <div className="card" style={{ background: '#fff', border: '1px solid var(--gray-200)', borderLeft }}>
      <div className="row" style={{ gap: 10, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        <div className="action-grid" style={{ minWidth: 220 }}>
          {item?.video_link ? (
            <button className="button" type="button" onClick={() => window.open(item.video_link, '_blank', 'noopener,noreferrer')}>
              Open Video
            </button>
          ) : null}
          ...
          {Array.isArray(item?.documents) && item.documents.length ? (
            <button
              className="button"
              type="button"
              onClick={() => window.open(String(item.documents[0] || ''), '_blank', 'noopener,noreferrer')}
            >
              Open Document
            </button>
          ) : null}
            ...
            {item.documents
              ...
              .map((u) => (
                <button
                  ...
                  className="button"
                  type="button"
                  onClick={() => window.open(String(u), '_blank', 'noopener,noreferrer')}
                >
                  Open
                </button>
              ))}
