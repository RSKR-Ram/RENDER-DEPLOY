...
export function AdminPage() {
  ...
  async function onPresetMisExcelOnly() {
    ...
    try {
      ...
      // - PORTAL_EA_TECH: allow opening the module
      // - SECTION_EXCEL_MARKS: show Excel section
      // - TECH_PENDING_LIST: allow loading candidate list for the page
      // - ADMIN_EXCEL_MARKS_SUBMIT: allow Excel submit action
      await upsertPermissionsChunked_([
        { permType: 'UI', permKey: 'PORTAL_EA_TECH', rolesCsv: addRole_('', 'MIS'), enabled: true },
        { permType: 'UI', permKey: 'SECTION_EXCEL_MARKS', rolesCsv: addRole_('', 'MIS'), enabled: true },
        { permType: 'ACTION', permKey: 'TECH_PENDING_LIST', rolesCsv: addRole_('', 'MIS'), enabled: true },
        { permType: 'ACTION', permKey: 'ADMIN_EXCEL_MARKS_SUBMIT', rolesCsv: addRole_('', 'MIS'), enabled: true },
      ]);
      ...
      <div className="card">
        {role === 'ADMIN' ? (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
            ...
            <button className="button" type="button" onClick={() => navigate('/ea/tech')}>
              Open EA Tech
            </button>
          </div>
        ) : null}
              ...
              <tbody>
                {filteredPermissions.map((p) => (
                  <tr key={`${p.permType}:${p.permKey}`} style={{ borderTop: '1px solid #eee' }}>
                    ...
                    <td className="small">{p.updatedBy || '-'}</td>
                    <td>
                      {permRole ? (
                        <label className="small" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <input
