...
import { ownerCandidatesList, ownerDecide, ownerFinalDecide, holdRevert } from '../api/candidates';
          ...
          {active === 'APPROVE' ? (
            ...
            >
              {finalItems.length === 0 ? (
                ...
                <div style={{ display: 'grid', gap: 10 }}>
                  {finalItems.map((c) => {
                    const selectedTests = Array.isArray(c.techSelectedTests) ? c.techSelectedTests : [];
                    return (
                      <div key={c.candidateId} className="card" style={{ background: '#fff', border: '1px solid var(--gray-200)' }}>
                        <div className="row" style={{ justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'flex-start' }}>
                          <div style={{ minWidth: 280, flex: 1 }}>
                            ...
                            <div className="small" style={{ color: 'var(--gray-500)', marginTop: 2 }}>
                              {c.jobTitle ? `${c.jobTitle} · ` : ''}{c.jobRole || '-'} · {c.mobile || '-'}
                            </div>
                            <div style={{ marginTop: 8, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                              <span className="badge">Req: {c.requirementId}</span>
                            ...
                            <div style={{ marginTop: 12, padding: '10px 12px', background: 'var(--gray-100)', borderRadius: 'var(--radius)' }}>
                              ...
                              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                ...
                                >
                                  Online Test: {c.onlineTestScore || '-'}{c.onlineTestResult ? ` (${String(c.onlineTestResult).toUpperCase()})` : ''}
                                </span>
                              ...
                              {selectedTests.length > 0 && (
                                <div className="small" style={{ marginTop: 6 }}>
                                  Tech Tests: {selectedTests.join(', ')}
                                </div>
                              )}
                              <div className="small" style={{ marginTop: 4 }}>
                                Marks:
                                {' '}Tally {c.tallyMarks || '-'}{pf10_(c.tallyMarks) ? ` (${pf10_(c.tallyMarks)})` : ''}
                                {' '}· Voice {c.voiceMarks || '-'}{pf10_(c.voiceMarks) ? ` (${pf10_(c.voiceMarks)})` : ''}
                                {' '}· Excel {c.excelMarks || '-'}{pf10_(c.excelMarks) ? ` (${pf10_(c.excelMarks)})` : ''}
                              </div>
                              <div className="small" style={{ marginTop: 4 }}>
                                Tech Result: <strong>{String(c.techResult || '-').toUpperCase()}</strong>
                                {c.techEvaluatedAt && ` · ${new Date(c.techEvaluatedAt).toLocaleString()}`}
                                {String(c.techResult || '').toUpperCase() === 'FAIL' && isContinued_(c, 'TECHNICAL') ? (
                                  <span style={{ marginLeft: 8, color: 'var(--gray-600)' }}>(Continued by HR)</span>
                                ) : null}
                              </div>
                              {(c.techReview || c.excelReview) ? (
                                <div className="small" style={{ marginTop: 4 }}>
                                  {c.techReview ? <div>Tech Review: {c.techReview}</div> : null}
                                  {c.excelReview ? <div>Excel Review: {c.excelReview}</div> : null}
                                </div>
                              ) : null}
                            </div>
                          </div>
        ...
        >
          ...
          >
            ...
            <div className="small" style={{ color: 'var(--gray-500)', marginBottom: 16 }}>
              {modal.candidate?.candidateName} ({modal.candidate?.candidateId})
            </div>
