  ...
  switch (role) {
    ...
    case 'EA':
      return [
        ...
        { key: 'tech', label: 'Pending Technical Tests' },
        { key: 'rejections', label: 'Rejection Log' },
      ];
    case 'HR':
      return [
...
export function DashboardPage() {
  ...
  const tabs = useMemo(() => {
    ...
    if (permissions) {
      ...
      add('tech', 'EA Tech', 'PORTAL_EA_TECH');
    ...
    async function loadOverview() {
      ...
      try {
        ...
        if (
          ...
          allowPortal_('PORTAL_EA_TECH', legacy === 'HR' || legacy === 'ADMIN') ||
          ...
          const [precall, inperson, tech, final, finalHold, joining, probation] = await Promise.all([
            precallList(token, { countOnly: true }),
            inpersonPipelineList(token, { countOnly: true }),
            techPendingList(token, { countOnly: true }),
      ...
      {overview?.hr && (
        <div className="card">
          ...
          <div className="metric-grid">
            ...
            <MetricCard value={overview.hr.techPending} label="Tech Pending" highlight={overview.hr.techPending > 0} onClick={() => navigate('/ea/tech')} />
  ...
  const renderContent = () => {
    ...
    if (active === 'tech' && (role === 'EA' || isAdmin)) {
      return renderActionPage('Technical Tests', 'Enter technical, voice, and tally marks. Excel marks are admin-only.', '💻', 'orange', '/ea/tech', 'Open Tech Tests');
    }
    ...
    if (active === 'inperson' && (role === 'HR' || isAdmin)) {
      return renderActionPage('In-person Interview', 'Enter in-person marks (auto-reject if <6), select technical tests.', '🏢', 'blue', '/hr/inperson', 'Open In-person');
    }

    if (active === 'final' && (role === 'HR' || isAdmin)) {
      return renderActionPage('Final Interview', 'Review candidates who passed technical tests.', '🎯', 'green', '/hr/final', 'Open Final Interview');
    }
