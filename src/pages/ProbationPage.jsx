...
import { assignTraining, trainingList, trainingMasterList, updateTrainingStatus } from '../api/training';
...
export function ProbationPage() {
  ...
  function canCompleteProbation_(candidateId) {
    const t = trainingsByCandidate?.[candidateId];
    const c = t?.counts;
    if (!c || !c.TOTAL) return { ok: false, reason: 'Training not assigned' };
    const open = (c.PENDING || 0) + (c.IN_PROGRESS || 0) + (c.OVERDUE || 0);
                  ...
                  <div key={`${it.requirementId}:${it.candidateId}`} className="card" style={{ background: '#fff', border: '1px solid var(--gray-200)' }}>
                    ...
                    <div className="row" style={{ alignItems: 'flex-start', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
                      <div style={{ flex: 1, minWidth: 240 }}>
                        <div style={{ fontWeight: 700, fontSize: '16px' }}>{it.candidateName || it.candidateId}</div>
                        ...
                        <div className="row" style={{ gap: 8, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                          ...
                          {!canCompleteProbation_(it.candidateId).ok ? (
                            <div className="small" style={{ marginTop: 6, color: 'var(--gray-600)' }}>
                              Probation complete disabled: {canCompleteProbation_(it.candidateId).reason}. (Finish trainings first)
                            </div>
                          ) : null}
                    ...
                    <div style={{ padding: '12px', background: 'var(--gray-50)', borderRadius: 'var(--radius)' }}>
                        ...
                        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          ...
                          {tItems.length ? (
                            <button
                              ...
                              onClick={async () => {
                                const lines = [
                                  `Candidate: ${it.candidateName || it.candidateId}`,
                      ...
                      {canAssignTraining ? (
                        <div style={{ marginTop: 12 }}>
                          ...
                          <div style={{ height: 10 }} />
                          <div>
                            <label className="small" style={{ display: 'block', marginBottom: 4 }}>Documents (one URL per line, optional)</label>
                            <textarea
