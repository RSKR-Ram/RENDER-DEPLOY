import{a as q,r as m,j as e,A as B,aj as D,n as j,ak as U}from"./index-Dn7xk2-6.js";function g(a){if(!a)return"";const c=a instanceof Date?a:new Date(a);return Number.isNaN(c.getTime())?String(a):c.toLocaleString()}function _(a,c){const h=new Blob([JSON.stringify(c,null,2)],{type:"application/json"}),i=URL.createObjectURL(h),n=document.createElement("a");n.href=i,n.download=a,document.body.appendChild(n),n.click(),n.remove(),URL.revokeObjectURL(i)}function s(a){return String(a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}function O(a){const c=window.open("","_blank","noopener,noreferrer");if(!c){j.error("Popup blocked");return}const i=(Array.isArray(a.logs)?a.logs:[]).map(n=>`<tr>
          <td>${s(g(n.at))}</td>
          <td>${s(n.stageTag)}</td>
          <td>${s(n.remark)}</td>
          <td>${s(n.actorRole)}</td>
