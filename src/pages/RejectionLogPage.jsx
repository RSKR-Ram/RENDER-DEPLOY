...
<body>
  <h2>Rejection Log</h2>
  <div class="small">Candidate: ${escapeHtml(it.candidateName)} (${escapeHtml(it.candidateId)})</div>
  <div class="small">Requirement: ${escapeHtml(it.requirementId)} ${it.jobTitle ? `· ${escapeHtml(it.jobTitle)}` : ''}</div>
  <div class="small">Role: ${escapeHtml(it.jobRole)} · Mobile: ${escapeHtml(it.mobile)} · Source: ${escapeHtml(it.source)}</div>
        ...
        <div className="row" style={{ gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '280px' }}>
            <input
              ...
              placeholder="🔍 Search by Name, Mobile, ID, Stage..."
              style={{ width: '100%' }}
            />
          </div>
                  ...
                  <div className="row" style={{ justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                        <div style={{ fontWeight: 600, fontSize: '16px' }}>{it.candidateName || it.candidateId}</div>
                      ...
                      <div className="small" style={{ display: 'grid', gap: '4px' }}>
                        ...
                        <div><strong>Mobile:</strong> {it.mobile}</div>
